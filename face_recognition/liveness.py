...
class Liveness:
    def __init__(self, ear_threshold: float = 0.21, blink_consec_frames: int = 2, blinks_to_verify: int = 2):
        # blink detection parameters
        self.ear_threshold = ear_threshold # threshold for blink detection
        self.blink_consec_frames = blink_consec_frames # threshold for consecutive frames to confirm blink
        self.blinks_to_verify = blinks_to_verify # number of blinks to verify
        self.closed_eye_frames_counter = 0 # counter for consecutive frames with EAR below threshold
    ...
    def eye_aspect_ratio(self, is_right: bool, mesh_landmarks):
        ...
        A = np.linalg.norm(np.array(mesh_landmarks[eye_landmarks[1]]) - np.array(mesh_landmarks[eye_landmarks[5]]))
        B = np.linalg.norm(np.array(mesh_landmarks[eye_landmarks[2]]) - np.array(mesh_landmarks[eye_landmarks[4]]))
        C = np.linalg.norm(np.array(mesh_landmarks[eye_landmarks[0]]) - np.array(mesh_landmarks[eye_landmarks[3]]))
        return (A + B) / (2.0 * C)
    ...
    def calculate_liveness(self, mesh_landmarks):
        ...
        return self.total_blinks >= self.blinks_to_verify
